...
Analyze this video natively. The user has selected the category: "${category}". You must evaluate category suitability and provide specific insights based on this category.
...
2. Compute engagement scores and the 14 other requested metrics (Hook strength, Value, Information density, Delivery, Visual & Editing quality, etc.) on a 0-100 scale.
...
5. Analyse each scene for privacy risks, specifically detecting faces, and check overall safety/harmful content.
...
Return a JSON object with this exact structure:
{
  ...
  "resolution": "1080p",
  "fps": 30,
  "category_suitability": 85,
  "hook_strength": 90,
  "engagement_score": 88,
  "content_value": 80,
  "information_density": 75,
  "delivery_strength": 92,
  "visual_quality": 85,
  "editing_quality": 78,
  "emotional_impact": 82,
  "content_uniqueness": 70,
  "safety_score": 100,
  "virality_score": 85,
  "virality_prediction": "High Viral Potential",
  "competitor_benchmark": 80,
  "competitor_benchmark_text": "Your video pacing ...",
  "ai_improvements": ["Increase hook strength...", "Improve lighting..."],
  "scenes": [
    { 
      "id": 1, 
      "timestamp": "0:00-0:15", 
...
- All 15 overarching metrics (0-100 scale) and AI improvements array MUST be included.
...
- Return ONLY the JSON object, no other text.`;
            ...
            if (parsed) {
                ...
                const result = {
                    ...
                    results: { 
                        duration: parsed.duration || 'N/A', resolution: parsed.resolution || 'N/A', fps: parsed.fps || 30, scenes, thumbnails,
                        category_suitability: parsed.category_suitability || 0,
                        hook_strength: parsed.hook_strength || 0,
                        engagement_score: parsed.engagement_score || 0,
                        content_value: parsed.content_value || 0,
                        information_density: parsed.information_density || 0,
                        delivery_strength: parsed.delivery_strength || 0,
                        visual_quality: parsed.visual_quality || 0,
                        editing_quality: parsed.editing_quality || 0,
                        emotional_impact: parsed.emotional_impact || 0,
                        content_uniqueness: parsed.content_uniqueness || 0,
                        safety_score: parsed.safety_score || 0,
                        virality_score: parsed.virality_score || 0,
                        virality_prediction: parsed.virality_prediction || 'Unknown',
                        competitor_benchmark: parsed.competitor_benchmark || 0,
                        competitor_benchmark_text: parsed.competitor_benchmark_text || 'No data',
                        ai_improvements: parsed.ai_improvements || []
                    },
        ...
        <div className="page-enter">
            <div className="page-header">
                ...
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
                    Upload a video to analyze scenes, engagement, and find the best thumbnails
                </motion.p>
            </div>
                ...
                {phase === 'upload' && (
                    ...
                    >
                        <div style={{ marginBottom: 'var(--space-xl)' }}>
                            <label style={{ display: 'block', fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 'var(--space-xs)' }}>
                            ...
                            <select 
                                ...
                                style={{ width: '100%', height: 48, background: 'var(--bg-secondary)', color: 'var(--text-primary)' }}
                            >
    ...
    return (
        <>
            ...
            <div className="bento-grid mb-xl" style={{ gridTemplateRows: 'auto' }}>
                ...
                <div className="glass-card flex flex-col items-center justify-center gap-md">
                    <div className="flex gap-xl w-full justify-center">
                        <div className="flex flex-col items-center">
                            <CircularProgress percentage={r.engagement_score || avgEngagement} label="Engagement" />
                        </div>
                        <div className="flex flex-col items-center">
                            <CircularProgress percentage={r.virality_score || 0} label="Virality Score" />
                        </div>
                    </div>
                    {r.virality_prediction && (
                        <div style={{ marginTop: 'var(--space-sm)', fontSize: '0.95rem', color: 'var(--text-primary)', textAlign: 'center', fontWeight: 600 }}>
                            Prediction: <span style={{ color: 'var(--lavender)' }}>{r.virality_prediction}</span>
                        </div>
                    )}
                    <div style={{ marginTop: 'var(--space-xs)', fontSize: '0.8rem', color: 'var(--text-tertiary)', textAlign: 'center', maxWidth: '80%' }}>
                        Overall viewer retention and viral prediction based on AI analysis.
                    </div>
                </div>
                ...
                <div className="glass-card flex flex-col justify-center gap-lg">
                    ...
                    <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 'var(--space-lg)' }}>
                        ...
                        {data.recommendations?.length > 0 && data.recommendations.map((rec, i) => (
                            <div key={`rec-${i}`} className="flex items-start gap-sm mb-sm" style={{ background: 'var(--glass-bg-active)', padding: '10px 12px', borderRadius: 'var(--radius-md)' }}>
                        ...
                        {r.ai_improvements?.length > 0 && r.ai_improvements.map((imp, i) => (
                            <div key={`imp-${i}`} className="flex items-start gap-sm mb-sm" style={{ background: 'var(--glass-bg-active)', padding: '10px 12px', borderRadius: 'var(--radius-md)' }}>
                                <Target size={14} color="var(--warning)" style={{ marginTop: 2, flexShrink: 0 }} />
                                <span style={{ fontSize: '0.8rem', color: 'var(--text-primary)', lineHeight: 1.4 }}>{imp}</span>
                            </div>
                        ))}

                        {(!data.recommendations?.length && !r.ai_improvements?.length) && (
                            <div style={{ fontSize: '0.8rem', color: 'var(--text-tertiary)', fontStyle: 'italic', padding: '10px 12px', background: 'var(--glass-bg-active)', borderRadius: 'var(--radius-md)' }}>No explicit highlights or improvements available.</div>
                        )}
                    </div>
                </div>
            </div >

            <div className="section-title"><span className="section-icon"><Activity size={18} color="var(--purple-light)" /></span> Detailed Analysis Metrics</div>
            <div className="glass-card-static mb-xl" style={{ padding: 'var(--space-2xl) var(--space-xl)' }}>
                <div style={{ 
                    display: 'grid', 
                    ...
                    {[
                        { label: 'Category Match', val: r.category_suitability },
                        { label: 'Hook Strength', val: r.hook_strength },
                        { label: 'Content Value', val: r.content_value },
                        { label: 'Info Density', val: r.information_density },
                        { label: 'Delivery', val: r.delivery_strength },
                        { label: 'Visuals', val: r.visual_quality },
                        { label: 'Editing', val: r.editing_quality },
                        { label: 'Emotion', val: r.emotional_impact },
                        { label: 'Uniqueness', val: r.content_uniqueness },
                        { label: 'Competitor', val: r.competitor_benchmark },
                        { label: 'Safety', val: r.safety_score },
                        { label: 'Overall Quality', val: Math.round(((r.visual_quality || 0) + (r.editing_quality || 0) + (r.content_value || 0)) / 3) || 50 } 
                    ].map((m, i) => (
                        <div key={i} className="flex flex-col items-center">
                            <CircularProgress percentage={m.val || 0} label={m.label} />
                        </div>
                    ))}
                </div>
                
                {r.competitor_benchmark_text && (
                    <div style={{ marginTop: 'var(--space-2xl)', padding: 'var(--space-lg)', background: 'var(--glass-bg-active)', borderRadius: 'var(--radius-md)', fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
                        <strong style={{ color: 'var(--text-primary)' }}>Competitor Benchmark Analysis:</strong> {r.competitor_benchmark_text}
                    </div>
                )}
            </div>
